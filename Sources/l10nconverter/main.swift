...
let outputOption = Option<String>(
  "output", ".", flag: "o",
  description: "The path to the folder where to output the results (Omit to generate in current working directory)"
)
...
let convertAndroidFileCommand = command(
  ...
  print(output)
  _ = LocalizationConverter.convert(androidFileName: inputFile, outputPath: output, includePlurals: includePlurals)
}
...
let convertAndroidFolderCommand = command(
  ...
  _ = LocalizationConverter.convert(androidFolder: inputFolder, outputPath: output, includePlurals: includePlurals)
}

let main = Group {
  $0.addCommand("convertAndroidFile", "Read a given Android strings.xml file and generate the corresponding Localizable.strings and Localizable.stringsdict files for iOS.", convertAndroidFileCommand)
